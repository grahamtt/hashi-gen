"""Automatically adds/removes imports via autohooks hook."""
...
def _check_autoimport_installed() -> None:
    try:
...
def _get_autoimport_config(config: Config) -> Config:
...
def __get_include_from_config(config: Optional[Config]) -> Iterable[str]:
    ...
    autoimport_config = _get_autoimport_config(config)
    include = _ensure_iterable(autoimport_config.get_value("include", DEFAULT_INCLUDE))

    return include


def _get_autoimport_arguments(config: Optional[Config]) -> Iterable[str]:
    if not config:
...
def precommit(
    ...
    **kwargs,  # pylint: disable=unused-argument
    ...
    """Run `autoimport` to update dependencies.
    ...
    arguments.extend(_get_autoimport_arguments(config))
