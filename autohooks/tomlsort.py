"""Sorts the pyproject.toml via autohooks hook."""
...
def __check_toml_sort_installed() -> None:
    try:
...
def __get_toml_sort_config(config: Config) -> Config:
...
def ___get_include_from_config(config: Optional[Config]) -> Iterable[str]:
    if not config:
        return DEFAULT_INCLUDE

    toml_sort_config = __get_toml_sort_config(config)
    include = _ensure_iterable(toml_sort_config.get_value("include", DEFAULT_INCLUDE))

    return include


def __get_toml_sort_arguments(config: Optional[Config]) -> Iterable[str]:
    if not config:
...
def precommit(
    ...
    **kwargs,  # pylint: disable=unused-argument
    ...
    """Run `poetry sort` and `tomlsort` on commit.
    ...
    """
    __check_toml_sort_installed()

    include = ___get_include_from_config(config)
    ...
    ret = 0
    arguments = ["toml-sort"]
    arguments.extend(__get_toml_sort_arguments(config))
