...
def __get_build_config(config: Config) -> Config:
...
def __get_build_arguments(config: Optional[Config]) -> Union[List[str], Tuple[str]]:
    if not config:
        return DEFAULT_ARGUMENTS

    autoimport_config = __get_build_config(config)
...
def precommit(
    ...
    **kwargs,  # pylint: disable=unused-argument
    ...
    arguments.extend(__get_build_arguments(config))
