"""Updates the dependencies via autohooks hook."""
...
def __get_lock_dependencies_config(config: Config) -> Config:
    return config.get("tool", "autohooks", "plugins", "update_dependencies", "lock")


def __get_install_dependencies_config(config: Config) -> Config:
...
def __get_lock_dependencies_arguments(config: Optional[Config]) -> Iterable[str]:
    if not config:
        return DEFAULT_ARGUMENTS

    lock_dependencies_config = __get_lock_dependencies_config(config)
...
def __get_install_dependencies_arguments(config: Optional[Config]) -> Iterable[str]:
    if not config:
        return DEFAULT_ARGUMENTS

    lock_dependencies_config = __get_install_dependencies_config(config)
    arguments = _ensure_iterable(
        lock_dependencies_config.get_value("arguments", DEFAULT_ARGUMENTS)
    )
...
def precommit(
    ...
    **kwargs,  # pylint: disable=unused-argument
    ...
    with stash_unstaged_changes():
        ...
        arguments.extend(__get_lock_dependencies_arguments(config))
        ...
        arguments.extend(__get_install_dependencies_arguments(config))
